...
trained_model_path = '/u/training/tra272/scratch/hw4/cifar_async/cifar_async.t7'
save_model_path = '/u/training/tra272/scratch/hw4/cifar_async'
...
for epoch in range(intial_epoch, num_epochs + intial_epoch):
    ...
    for i, input_batch in enumerate(trainloader, 0):
        ...
        loss.backward()  # Backpropagation
          # Updating the parameters using ADAM optimizer

        for param in model.parameters():

            if type(param.grad) == type(None):
                #tensor0 = Variable(torch.zeros(param.shape)).data.cpu()
                continue
    ...
    if epoch % 5 == 4:
        ...
        x1.to_csv('cifar_async_accuracy.csv')
